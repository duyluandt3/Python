...
##
#  @brief create the filter string of sevral packets for thark's option
#  @param d_tlv_param       TLV parameter \n
#      **["NumOfMsg"]**     the number of the message to be analyzed (int) \n
#      **["AnalyzeMsg"]**   AnalyzeMsg type which is defined
#                           in "CLS_Define.py" file
#  @param logger            logger parameter for debug
#  @retval s_filter         the packet filter set for thark's option
def create_pkt_filter(d_tlv_param, logger):
    logger.debug("[PKT CHK] create packet filter")
    ...
    for num in range(i_num_of_msg):
        ...
        if i_num_of_msg == 1:
            # mandatory parameter
            i_analyzemsg = d_tlv_param["AnalyzeMsg"]
        else:
...
def create_filter(d_tlv_param, i_analyzemsg, logger):
    ...
    s_filter += set_pkt_filter(i_analyzemsg, logger)
    # s_filter += create_pkt_filter(d_tlv_param, logger)
...
def compare_ie_val(i_analyzemsg, l_ie_name_list, d_ref_data, d_cap_ie,
    ...
    for s_ie_name in l_ie_name_list:
        ...
        # check l_ref_ie_val which is included in s_cap_ie_val
            ...
            d_rply_tlv["DataList"][i_cnt] = d_list_tlv

            i_cnt += 1
            d_rply_tlv["NumOfNg"] = i_cnt

    return i_cnt, d_rply_tlv
